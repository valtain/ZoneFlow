...
1. `issues/explorations/<name>/` 폴더가 없으면 아래 4개 파일을 생성한다.
...
인자가 슬래시 없는 이름이면 `issues/explorations/<name>/`을 로드해 논의를 이어간다.
...
3. `issues/features/<feature-name>/` 폴더를 생성하고 `spec.md` 초안을 candidate 내용으로 채운다.
...
7. `discussion.md`에 아래 형식으로 append한다:

```text
- [YYYY-MM-DD | promote] <Candidate Label> → issues/features/<feature-name>/ 생성
```
